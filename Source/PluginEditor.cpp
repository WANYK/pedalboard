...
ChoursAudioProcessorEditor::ChoursAudioProcessorEditor (ChorusAudioProcessor& p)
    : AudioProcessorEditor (&p), processor (p)
{
    const Array<AudioProcessorParameter*> parameters = processor.getParameters();
    int comboBoxCounter = 0;

    int editorHeight = 2 * editor_margin;
        ...
        if (const AudioProcessorParameterWithID* parameter =
            ...
            if (processor.parameters.parameterTypes[i] == "Slider") {
                ...
                aSlider->setTextBoxStyle(Slider::TextBoxLeft,
                    false,
                    slider_text_entry_box_width,
                    slider_text_entry_box_height);
    ...
    editorHeight += components.size() * editor_padding;
    setSize(editor_width, editorHeight);
}

ChoursAudioProcessorEditor::~ChoursAudioProcessorEditor()
{
}

//==============================================================================
//wersja graficzna
void ChoursAudioProcessorEditor::paint (Graphics& g)
{
    g.fillAll (getLookAndFeel().findColour (ResizableWindow::backgroundColourId));
}

//rozmiar tego komponentu zostały zmienione
void ChoursAudioProcessorEditor::resized()
{
    Rectangle<int> r = getLocalBounds().reduced(editor_margin);
    r = r.removeFromRight(r.getWidth() - label_width);

    for (int i = 0; i < components.size(); ++i) {
        if (Slider* aSlider = dynamic_cast<Slider*> (components[i]))
//Zmienia pozycję i rozmiar komponentu
            components[i]->setBounds(r.removeFromTop(slider_height));

        if (ToggleButton* aButton = dynamic_cast<ToggleButton*> (components[i]))
//Zmienia pozycję i rozmiar komponentu
            components[i]->setBounds(r.removeFromTop(button_height));

        if (ComboBox* aComboBox = dynamic_cast<ComboBox*> (components[i]))
//Zmienia pozycję i rozmiar komponentu
            components[i]->setBounds(r.removeFromTop(combo_box_height));

        r = r.removeFromBottom(r.getHeight() - editor_padding);
    }
}
